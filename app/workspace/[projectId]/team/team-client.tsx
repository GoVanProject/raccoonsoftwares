...
export default function TeamClient({ projectId }: { projectId: string }) {
  ...
  }, [clearToken, projectId, router, token]);

  async function addMember(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    if (!token || !memberEmail.trim()) return;
    try {
      await taskboardFetch(`/api/projects/${projectId}/members`, token, {
        method: "POST",
        body: JSON.stringify({ email: memberEmail }),
      });
      const response = await taskboardFetch<{ members: Member[] }>(
        `/api/projects/${projectId}/members`,
        token,
      );
        ...
        <section className="workspace-main">
          ...
          {loading ? (
            ...
            >
              ...
              <div className="workspace-skeleton workspace-skeleton-column" />
            </div>
          ) : project ? (
            <div className="workspace-team-view">
              <header className="workspace-view-header">
                <div className="workspace-view-heading">
                  <span className="workspace-kicker">Projeto ativo</span>
                  <h1>Equipe</h1>
                  <p>
                    Gerencie as pessoas que colaboram em{" "}
              ...
              <div className="workspace-team-summary">
                ...
                <div>
                  ...
                  <span>
                    Convide integrantes e defina o nível de acesso de cada um.
                  </span>
                </div>
              </div>
              <section
                className="workspace-team-card"
                aria-labelledby="workspace-team-members-title"
              >
                <div className="workspace-team-card-header">
                  <div>
                    <span className="workspace-kicker">Acesso do projeto</span>
                    <h2 id="workspace-team-members-title">Membros</h2>
                  </div>
                  <span className="workspace-team-count">{members.length}</span>
                </div>
                  ...
                  {members.map((member) => (
                    <article className="workspace-member-row" key={member.id}>
                      <span className="workspace-member-avatar">
                        {member.avatar_data ? (
                          <Image src={member.avatar_data} alt="" width={40} height={40} unoptimized />
                        ) : (
                      ...
                      <div className="workspace-member-identity">
                        <strong>{member.alias || member.email}</strong>
                        <small>
                          {member.alias
                            ? member.email
                      ...
                      <select
                        value={
                          member.role ||
                          (member.id === project.owner_id ? "owner" : "editor")
                        }
                        disabled={member.id === project.owner_id || !canManage}
                        onChange={(event) =>
                      ...
                      >
                        ...
                        <option value="editor">
                          {memberRoleLabels.editor}
                        </option>
                        <option value="viewer">
                          {memberRoleLabels.viewer}
                        </option>
                      </select>
                      {member.id !== project.owner_id ? (
                        <ConfirmDialog
                          title="Remover integrante?"
                          description={`${member.alias || member.email} perderá o acesso a este projeto.`}
              ...
              <form
                className="workspace-member-form workspace-member-form-card"
                onSubmit={addMember}
              >
                <div>
                  <span className="workspace-kicker">Adicionar integrante</span>
                  <h2>Convide alguém para o projeto</h2>
                  <p>
                    A pessoa precisa ter uma conta cadastrada para receber
                    acesso.
                  </p>
                </div>
                <div className="member-form">
                  <input
                    ...
                    aria-label="Email do integrante"
                    disabled={!canManage}
                    required
                  />
