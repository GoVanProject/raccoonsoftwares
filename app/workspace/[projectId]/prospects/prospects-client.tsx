...
import { ConfirmDialog } from "@/components/workspace/workspace-primitives";
import { useTaskboardToken } from "../../../lib/use-taskboard-token";
const LeadMap = dynamic(() => import("./lead-map"), {
  ssr: false,
  loading: () => (
    <div
      className="prospects-map prospects-map-loading"
...
function LeadPagination({
  ...
  return (
    <nav
      className="prospects-pagination"
      aria-label="Paginação de restaurantes"
    >
      <button
        type="button"
...
export default function ProspectsClient({ projectId }: { projectId: string }) {
  ...
  const loadLeadData = useCallback(
    async (accessToken: string) => {
      setLoading(true);
      try {
        const projectListResponse = await taskboardFetch<{
          projects: Project[];
        }>("/api/projects", accessToken);
        if (
          !projectListResponse.projects.some((item) => item.id === projectId)
        ) {
          router.replace("/workspace?error=project-not-found");
          return;
        }
        const [projectResponse, memberResponse, leadResponse, userResponse] =
          await Promise.all([
            ...
            taskboardFetch<{ leads: Lead[] }>(
              `/api/projects/${projectId}/leads`,
              accessToken,
            ),
            taskboardFetch<{ user: { id: string } }>(
              "/api/auth/me",
              accessToken,
            ),
          ]);
        ...
        if (reason instanceof Error && reason.message.includes("token")) {
          ...
          (reason.message.includes("acesso") ||
            reason.message.includes("encontrado"))
        ) {
          router.replace("/workspace?error=project-not-found");
        } else {
          setError(
  ...
  useEffect(() => {
    ...
    taskboardFetch<{ activities: Activity[] }>(
      `/api/leads/${selectedLeadID}/activities`,
      token,
    )
  ...
  const cityOptions = useMemo(
    () =>
      [...new Set(leads.map((lead) => lead.city))].sort((a, b) =>
  ...
  async function saveLead(event: FormEvent<HTMLFormElement>) {
    ...
    const payload = {
      ...(current
        ? {
            ...
            location_precision: current.location_precision,
          }
        : {}),
      ...draft,
    ...
    try {
      const response = current
        ? await taskboardFetch<{ lead: Lead }>(
            `/api/leads/${current.id}`,
            token,
      ...
      setLeads((currentLeads) =>
        current
          ? currentLeads.map((lead) =>
              lead.id === current.id ? response.lead : lead,
            )
          : [...currentLeads, response.lead],
      );
  ...
  async function importSeed() {
    ...
    setError("");
    try {
      const response = await taskboardFetch<{
        leads: Lead[];
        created: number;
        skipped: number;
      }>(`/api/projects/${projectId}/leads/import`, token, {
        method: "POST",
        body: JSON.stringify({ leads: seedLeads }),
      });
      setLeads((current) => [...current, ...response.leads]);
      setNotice(
        `Base importada: ${response.created} novos e ${response.skipped} já existentes.`,
      );
    } catch (reason) {
      setError(
        reason instanceof Error
          ? reason.message
          : "Não foi possível importar a base.",
      );
    } finally {
      setImporting(false);
    }
  ...
  function handleProspectColumnDragOver(
  ...
  function handleProspectColumnDragLeave(
    event: DragEvent<HTMLElement>,
    nextStatus: Status,
  ) {
  ...
  async function moveLeadToStatus(lead: Lead, nextStatus: Status) {
    ...
    try {
      const response = await taskboardFetch<{ lead: Lead }>(
        `/api/leads/${lead.id}`,
        token,
        {
          method: "PATCH",
      ...
      setLeads((current) =>
        current.map((item) => (item.id === lead.id ? response.lead : item)),
      );
    } catch (reason) {
      setLeads((current) =>
        current.map((item) =>
  ...
  function handleProspectColumnDrop(
    event: DragEvent<HTMLElement>,
    nextStatus: Status,
  ) {
    event.preventDefault();
  ...
  async function addActivity(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await taskboardFetch<{ activity: Activity }>(
        ...
        token,
        {
          ...
          body: JSON.stringify({
            type: activityType,
            body: activityBody,
            status_after: activityStatus || undefined,
          }),
        },
      );
  ...
  async function deleteSelected() {
    if (!token || !selectedLead || readOnly) return;
    try {
      await taskboardFetch(`/api/leads/${selectedLead.id}`, token, {
        method: "DELETE",
      });
      setLeads((current) =>
  ...
  function downloadCSV() {
    ...
    const lines = [
      ...
      ...filteredLeads.map((lead) => [
        ...
        displayMember(members.find((member) => member.id === lead.assignee_id)),
  ...
  if (loading) {
    return (
      <main className="workspace-page">
        <div className="workspace-loading prospects-loading">
          <div className="workspace-skeleton workspace-skeleton-title" />
          <div className="workspace-skeleton prospects-loading-map" />
        </div>
      </main>
    );
  }
    ...
    <main className="workspace-page prospects-page">
      <div className="workspace-layout">
        <section className="prospects-main">
          ...
          {notice ? (
            <div className="prospects-notice" role="status">
              {notice}
          ...
          <header className="prospects-heading">
            <div>
              <Link className="prospects-back" href={`/workspace/${projectId}`}>
                <WorkspaceIcon name="back" />
                Voltar ao quadro
              </Link>
              <span className="workspace-kicker">
                {project?.name || "Projeto"} · prospecção
              </span>
              <h1>Mapa de restaurantes</h1>
              <p>
                Veja onde estão os negócios e organize o próximo contato
            ...
            <div className="prospects-actions">
              ...
              >
                ...
                {importing ? "Importando…" : "Importar base inicial"}
              </button>
              <button
          ...
          >
            ...
            <div>
              <strong>
                <NumberTicker value={metrics.new} />
              </strong>
              <span>novos</span>
            </div>
            <div>
              <strong>
                <NumberTicker value={metrics.noResponse} />
              </strong>
              <span>sem retorno</span>
            </div>
            <div>
              <strong>
                <NumberTicker value={metrics.customers} />
              </strong>
              <span>clientes</span>
            </div>
          </section>
          ...
          <section
            className="prospects-filters"
            aria-label="Filtros da prospecção"
          >
            <label className="prospects-search">
              <span>Buscar</span>
              <input
                value={search}
          ...
          <div className="prospects-view-bar">
            <div>
              <strong>{filteredLeads.length}</strong>{" "}
              {filteredLeads.length === 1 ? "resultado" : "resultados"}
            </div>
            <Tabs
              value={view}
              onValueChange={(value) => setView(value as "map" | "pipeline")}
            >
              <TabsList className="prospects-view-toggle">
                <TabsTrigger value="map">
                  <WorkspaceIcon name="mapPin" />
                  Mapa
                </TabsTrigger>
          ...
          {view === "map" ? (
            <div className="prospects-map-layout">
              <LeadMap
                leads={visibleLeads}
              ...
              <aside
                ...
                aria-label="Restaurantes filtrados"
              >
                <div className="prospects-list-header">
                  <span>Restaurantes</span>
                  <small>Selecione um ponto para ver detalhes</small>
                </div>
                {visibleLeads.map((lead) => (
                  <button
                    className={`prospect-row ${selectedLeadID === lead.id ? "active" : ""}`}
                    type="button"
                    key={lead.id}
                    onClick={() => setSelectedLeadID(lead.id)}
                  >
                    <span className={`prospect-status-dot ${lead.status}`} />
                    <span className="prospect-row-copy">
                      <strong>{lead.name}</strong>
                      <small>
                        {lead.city} · {lead.category || "Sem categoria"}
                      </small>
                    </span>
                    <span className="prospect-row-arrow"><WorkspaceIcon name="chevron" /></span>
                  </button>
                ))}
            ...
            <div className="prospects-pipeline">
              {statuses.map((column) => {
                ...
                return (
                  <section
                    className={`prospects-column ${dragOverStatus === column.value ? "is-drag-over" : ""}`}
                    key={column.value}
                    onDragOver={(event) =>
                      handleProspectColumnDragOver(event, column.value)
                    }
                    onDragLeave={(event) =>
                      handleProspectColumnDragLeave(event, column.value)
                    }
                    onDrop={(event) =>
                      handleProspectColumnDrop(event, column.value)
                    }
                  >
                    <div className="prospects-column-heading">
                      <span className={`prospect-status-dot ${column.value}`} />
                      <h2>{column.label}</h2>
                      <b>{columnLeads.length}</b>
                    </div>
                    <div className="prospects-column-stack">
                      {columnLeads.map((lead) => (
                        <article
                          className={`prospect-pipeline-card ${draggedLeadID === lead.id ? "is-dragging" : ""} ${movingLeadID === lead.id ? "is-moving" : ""}`}
                          key={lead.id}
                          draggable={!readOnly}
                          onDragStart={(event) =>
                        ...
                        >
                          <button
                            className="prospect-pipeline-card-open"
                            type="button"
                            onClick={() => setSelectedLeadID(lead.id)}
                          >
                            <strong>{lead.name}</strong>
                            ...
                            {lead.next_contact_at ? (
                              <small>
                                Retorno em {formatDate(lead.next_contact_at)}
                              </small>
                            ) : null}
                          </button>
                          <select
                            className="prospect-pipeline-status"
                            aria-label={`Mover ${lead.name} para etapa`}
                            value={lead.status}
                            disabled={readOnly || movingLeadID === lead.id}
                            onChange={(event) =>
                              void moveLeadToStatus(lead, event.target.value as Status)
                            }
                          >
                            {statuses.map((option) => (
                      ...
                      {!columnLeads.length ? (
                        <div className="prospects-column-empty">
                          Nenhum lead
                        </div>
                      ) : null}
                    </div>
          ...
          {view === "pipeline" && filteredLeads.length > PAGE_SIZE ? (
            <LeadPagination
              page={currentPage}
              totalPages={totalPages}
              onPageChange={setPage}
      ...
      >
        {selectedLead ? (
          <DialogContent
            className="workspace-modal prospects-detail-modal"
          ...
          >
            <div className="workspace-modal-header">
              <div>
                ...
                <h2 id="prospect-detail-title">{selectedLead.name}</h2>
              </div>
              <button
            ...
            <div className="prospect-detail-content">
              ...
              <div className="prospect-detail-links">
                {selectedLead.phone &&
                !["Não localizado", "Não confirmado"].includes(
                  selectedLead.phone,
                ) ? (
                ...
                {selectedLead.website ? (
                  <a
                    ...
                    rel="noreferrer"
                  >
                    {selectedLead.website_label || "Website"}
                  </a>
                ) : null}
                {selectedLead.map_url ? (
                  <a
                    href={selectedLead.map_url}
                    target="_blank"
                    rel="noreferrer"
                  >
                    Abrir no Maps
              ...
              <p className="prospect-address">
                {selectedLead.address ||
                  ...
                  "Localização aproximada pela cidade"}
              </p>
              <div className="prospect-detail-grid">
                <div>
                  ...
                  <strong>
                    {selectedLead.assignee_id
                      ? displayMember(
                          members.find(
                            (member) => member.id === selectedLead.assignee_id,
                          ) as Member,
                        )
                      : "Sem responsável"}
                  </strong>
                ...
                <div>
                  ...
                  <strong>{formatDate(selectedLead.next_contact_at)}</strong>
                </div>
                <div>
                ...
                <div>
                  ...
                  <strong>
                    {selectedLead.rating
                      ? `★ ${selectedLead.rating.toFixed(1).replace(".", ",")}`
                      : "Não informada"}
                  </strong>
                </div>
              </div>
              ...
              <div className="prospect-detail-actions">
                ...
                <ConfirmDialog
                  trigger={
                    <button
                      className="edit-cancel-button danger-action"
                      type="button"
                      disabled={readOnly}
                    >
                      Excluir
                  ...
                  description={`O cadastro de ${selectedLead.name} será removido permanentemente.`}
                  confirmLabel="Excluir cadastro"
                  destructive
                  onConfirm={() => void deleteSelected()}
                />
              </div>
              <div className="prospect-history">
                <div className="prospect-history-heading">
                  <div>
                    <span className="workspace-kicker">Histórico</span>
                ...
                {!readOnly ? (
                  <form
                    className="prospect-activity-form"
                  ...
                  >
                    <div className="prospect-activity-fields">
                      ...
                      <select
                        ...
                        onChange={(event) =>
                          setActivityStatus(event.target.value as Status | "")
                        }
                ...
                <div className="prospect-activity-list">
                      ...
                      <div>
                        ...
                        <small>
                          ...
                          {activity.status_after
                            ? ` · ${statusLabel(activity.status_after)}`
                            : ""}
                        </small>
                      </div>
                      <p>{activity.body}</p>
                  ...
                  {!activities.length ? (
                    <p className="prospect-history-empty">
      ...
      <Dialog
        ...
        onOpenChange={(open) => {
          if (!open) setLeadModal(null);
        }}
      >
        {leadModal ? (
          <DialogContent
            className="workspace-modal prospect-form-modal"
            showCloseButton={false}
          ...
          >
            <div className="workspace-modal-header">
              <div>
                <span className="workspace-kicker">
                  {leadModal === "create" ? "Novo lead" : "Cadastro comercial"}
                </span>
            ...
            <form className="task-modal-form" onSubmit={saveLead}>
              <div className="prospect-form-grid">
                ...
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateDraft("preferred_channel", event.target.value)
                    }
                  >
                ...
                <label className="prospect-form-wide">
                  Endereço
                  <input
                    value={draft.address}
                    onChange={(event) =>
                ...
                <label>
                  ...
                  <select
                    ...
                    onChange={(event) =>
                      updateDraft("assignee_id", event.target.value)
                    }
                  >
                ...
                <label>
                  ...
                  <input
                    ...
                    onChange={(event) =>
                      updateDraft("next_contact_at", event.target.value)
                    }
                  />
                </label>
                <label>
                ...
                <label className="prospect-form-wide">
                  ...
                  <textarea
                    ...
                    rows={4}
                    placeholder="Contexto do contato, oportunidade e próximos passos"
                  />
                </label>
              </div>
