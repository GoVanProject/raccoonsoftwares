...
export default function WorkspacePage() {
  ...
  useEffect(() => {
    if (projectId) return;
    if (new URLSearchParams(window.location.search).get("error") === "project-not-found") {
  ...
  return (
    <main className="workspace-page">
      <div className="workspace-layout">
        ...
        <section className={`workspace-main ${!projectId ? "workspace-selection-main" : ""}`}>
          {error ? <div className="workspace-error" role="alert">{error}<button type="button" onClick={() => setError("")}>×</button></div> : null}
          {!projectId ? <div className="workspace-selection">
            <header className="workspace-selection-heading">
              ...
              <p>Quadro, prospecção, equipe e sala ao vivo ficam organizados dentro do projeto certo.</p>
            </header>
            <section className="workspace-selection-projects" aria-labelledby="workspace-selection-projects-title">
              <div className="workspace-selection-section-heading"><div><span className="workspace-kicker">Projetos disponíveis</span><h2 id="workspace-selection-projects-title">Onde você quer trabalhar?</h2></div><span>{loading ? "Carregando" : projects.length}</span></div>
              {loading ? <div className="workspace-selection-grid">{[1, 2, 3].map((item) => <div className="workspace-skeleton workspace-selection-card-skeleton" key={item} />)}</div> : null}
              {!loading && projects.length ? <div className="workspace-selection-grid">{projects.map((project) => <button className="workspace-selection-card" type="button" key={project.id} onClick={() => router.push(`/workspace/${project.id}`)}><span className="workspace-selection-card-icon">{project.image_data ? <img src={project.image_data} alt="" /> : <WorkspaceIcon name="folder" />}</span><span><strong>{project.name}</strong><small>{project.task_count} {project.task_count === 1 ? "tarefa" : "tarefas"}</small></span><WorkspaceIcon name="chevron" /></button>)}</div> : null}
              {!loading && !projects.length ? <div className="workspace-selection-empty"><WorkspaceIcon name="folder" /><strong>Nenhum projeto criado ainda.</strong><p>Crie o primeiro projeto para começar a organizar o trabalho da equipe.</p></div> : null}
            </section>
            <form className="workspace-selection-form" onSubmit={createProject}>
              <div><span className="workspace-kicker">Novo projeto</span><h2>Comece um novo espaço de trabalho</h2><p>O nome é obrigatório. Descrição e imagem são opcionais.</p></div>
              <div className="workspace-selection-form-fields"><label>Nome do projeto<input value={projectName} onChange={(event) => setProjectName(event.target.value)} placeholder="Ex.: Lançamento do site" aria-label="Nome do novo projeto" required /></label><label>Descrição<textarea value={projectDescription} onChange={(event) => setProjectDescription(event.target.value)} placeholder="Qual é o objetivo deste projeto?" aria-label="Descrição do novo projeto" rows={3} /></label><label className="image-picker"><span>🖼️ {projectImageName || "Anexar imagem"}</span><small>PNG, JPEG ou WebP · até 512 KB</small><input ref={projectImageInput} type="file" accept="image/png,image/jpeg,image/webp" onChange={handleProjectImage} aria-label="Imagem do projeto" /></label></div>
          ...
          </div> : loading ? <div className="workspace-loading" aria-busy="true" aria-label="Carregando workspace">
          ...
          </> : <div className="workspace-empty"><span>✦</span><h1>Projeto indisponível.</h1><p>Escolha outro projeto para continuar.</p><Link className="workspace-secondary-action" href="/workspace">Voltar para projetos</Link></div>}
        </section>
      </div>
